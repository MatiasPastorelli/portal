          ...
          <div class="col-lg-9 col-md-9">
            <div class="card">
                <div class="card-header">
                    <h5 class="subtitulo-isbast text-center">Actualizar Orientacion {{$orientaciones->nombreOrientacion}}</h5>
                </div>
                <form action="{{ asset('/orientacionUpdate') }}" method="post">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="idOrientacion" value="{{ $orientaciones->idOrientacion}}">
                    <div>
                      <div class="card-body m-3">
                            <div class="row">
                                <div class="form-group col-md-12">
                                    <label>Nombre plan<span class="text-danger">*</span></label>
                                    <input type="text" name="nombreOrientacion" value="{{$orientaciones->nombreOrientacion}}" class="form-control" >
                                </div>
                            </div>
                      </div>
                    </div>
